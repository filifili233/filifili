...
public class player extends JFrame {
	...
	public player() { // 构造函数
		...
		openFile.addActionListener(new ActionListener() { //打开文件按钮事件处理
			public void actionPerformed(ActionEvent event) {
				...
				int result = fileChooser.showOpenDialog(player.this);//显示对话框				
		...
		openURL.addActionListener(new ActionListener() { //打开URL按钮事件处理
			public void actionPerformed(ActionEvent event) {
				String addressName =JOptionPane.showInputDialog(player.this, "输入URL地址");
				if (addressName != null)
	...
	public void startPlayer(String mediaLocation) {
		...
		try {
			...
			((Controller) player).addControllerListener((ControllerListener) new PlayerEventHandler()); //增加播放控制器
			player.realize();
		} catch (Exception ex) {
			ex.printStackTrace();
			showMessage("打开错误"); //显示错误信息
